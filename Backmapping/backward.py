...
def reciter(filename):
    # Set the directory of the filename so we know where to expect #included
    # files
...
def dist(a, b, box=None, inv=None):
    ...
    #                                             |---difference vector---|
    return math.sqrt(
        norm2(mvmul(box, vr(mvmul(inv, [i - j for i, j in zip(a, b)])))))
...
def crossprod(a, b):
    return a[1] * b[2] - a[2] * b[1], a[2] * \
        b[0] - a[0] * b[2], a[0] * b[1] - a[1] * b[0]
...
def unbreak(R, box, inv=None):
    ...
    xyz = [vadd(R[0][4:7], mvmul(box, vr(mvmul(inv, vsub(i[4:7], R[0][4:7])))))
           for i in R]
...
def pdbAtom(a, strict=False):
    ...
    if strict:
        return (str(a[12:16]), str(a[17:20]), int(a[22:26]), a[21], float(
            a[30:38]) / 10, float(a[38:46]) / 10, float(a[46:54]) / 10)
    else:
        return (str(a[12:16]), str(a[17:21]), int(a[22:26]), a[21], float(
            a[30:38]) / 10, float(a[38:46]) / 10, float(a[46:54]) / 10)


def pdbBoxRead(a):
    fa, fb, fc, aa, ab, ac = [float(i) for i in a.split()[1:7]]
    ca, cb, cg, sg = math.cos(d2r *
                              aa), math.cos(d2r *
                                            ab), math.cos(d2r *
                                                          ac), math.sin(d2r *
                                                                        ac)
...
def pdbBoxString(b):
    ...
    return pdbBoxLine % (10 * norm(u), 10 * norm(v),
                         10 * norm(w), alpha, beta, gamma)
...
def groAtom(a):
    ...
    # ===>   atom name,   res name,     res id, chain,       x,          y,          z
    return (str(a[10:15]), str(a[5:10]), int(a[:5]), " ",
            float(a[20:28]), float(a[28:36]), float(a[36:44]))
...
class Structure:
    def __init__(self, other, strict=False):
        ...
        protein = [i[0][1].strip() in AminoAcids and get_calpha_xyz(i)
                   for i in self.residues]
        termini = [
            is_terminal(
                i, j, A, B) for i, j in zip(
                [False] + protein, protein + [False])]
        self.nterm = [j and i for i, j in zip(termini, protein)]
        self.cterm = [j and i for i, j in zip(termini[1:], protein)]

        # Set chain backbones based on termini. Begin with a 'chain' unless the
        # first residue is protein.
        backbone = [[]]
        for i, j, k in zip(self.nterm, self.cterm, self.residues):
            if i and backbone[-1]:
                # We have a chain start, and the last chain is not empty: add a
                # chain
                backbone.append([])
        ...
        # Maybe we just added an empty list to the backbone, like if the last
        # residue is a C-terminal
    ...
    def groBoxString(self):
        ...
        if self.box:
            return groBoxLine % (self.box[0][0],
                                 self.box[1][1],
                                 self.box[2][2],
                                 self.box[0][1],
                                 self.box[0][2],
...
class Topology:
    def __init__(self, other, out=None):
        ...
        # Process the topology file extract moleculetypes, atom lists and the
        # molecule list
        self.molecules = []
        self.top = []

        # Processed topology
        # This is equal to the input target topology, but with all #include
        # statements resolved
        if out:
            out = open(out, "w")
        ...
        molecules = [(i, len(list(j)))
                     for i, j in itertools.groupby(self.molecules)]
        ...
        self.atoms = [[a, r, i, c, 0, 0, 0, t]
                      for t, c in mr for a, r, i, m in self.moleculetypes[t]]
...
if '-h' in args or '--help' in args:
    ...
    for thing in options:
        print(not isinstance(thing, str) and "%10s  %s" %
              (thing[0], thing[1].description) or thing)
    print()
    sys.exit()
...
while args:
    ...
    options[opts[-1]].setvalue([args.pop(0)
                               for i in range(options[opts[-1]].num)])
...
if top:
    ...
    if options["-atomlist"]:
        ...
        atm.writelines("".join(["%6d %5s %5s\n" % (u, v[0], v[1])
                       for u, v in zip(list(range(1, len(topatm) + 1)), topatm)]))
...
for residue, bb, nterm, cterm in zip(
    ...
    if resn in list(solvent.keys()):
        ...
        for atom, x, y, z in solvent[resn]:
            ...
            if atom in ion_stuff:
                atom = atoms[0]
                ions.append(
                    (atom, resn, counter, chain, cx + x, cy + y, cz + z))
    ...
    if resn not in reslist:
        p = set(atoms)
        for i in reslist:
            if i.startswith(resn):
                if p == set([k for j in list(mapping[i].map.values())
                            for k in j]):
                    resn = i
                    break

    if resn not in list(mapping.keys()):
...
if backmapping:
    dev.write(
        "Backmapped structure from MARTINI to %s\n" %
        options["-to"].value)
else:
...
if options["-p"] and options["-po"]:
    ...
    po.writelines(["%s %5d\n" % (a, len(list(b)))
                  for a, b in itertools.groupby(sol)])

    ions = [(i[0], i[2]) for i in ions]
    ions = [a[0] for a, b in itertools.groupby(ions)]
    po.writelines(["%s %5d\n" % (a.replace("+", "").replace("-", ""),
                  len(list(b))) for a, b in itertools.groupby(ions)])
...
if options["-n"]:
    ...
    ndx.write("[ Protein ]\n" + "\n".join([str(i)
              for i in ndx_protein]) + "\n")
    ndx.write("[ Membrane ]\n" + "\n".join([str(i)
              for i in ndx_membrane]) + "\n")
    ndx.write("[ Solvent ]\n" + "\n".join([str(i)
              for i in ndx_solvent]) + "\n")
