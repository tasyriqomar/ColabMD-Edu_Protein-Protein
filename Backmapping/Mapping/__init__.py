    ...
    def do(self, residue, target=None, coords=False, nterm=False, cterm=False, nt=False, kick=0.05):
        ...
        for tag,i in self.mod:
            ...
            if tag == "trans":    
                ...
                flat_s = [item for tup in s for item in (tup if isinstance(tup, (list, tuple)) else [tup])]
                # Normalize s to integers (Python 3 safe)
                s_clean = []
                for x in s:
                    if isinstance(x, tuple):
                        s_clean.append(x[0])   # take first element
                    else:
                        s_clean.append(x)

                if s_clean and min(s_clean) > -1:
                ...
                s_clean = [x[0] if isinstance(x, tuple) else x for x in s]
                if min(s_clean) > -1:
