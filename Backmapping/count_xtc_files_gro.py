...
# Directory containing the .gro files
gro_directory = './gro'

# Get the list of all files in the gro directory
files = os.listdir(gro_directory)

# Regular expression to match the .gro files with the required pattern
pattern = re.compile(r'aa_(\d+)ns\.gro')
...
if not numbers:
    print("No valid .gro files found.")
else:
    ...
    # Output the results
    print(f"Number of .gro files: {num_files}")
